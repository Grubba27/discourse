...
const COMPOSER_EVENTS = [
  ["composer:insert-block", "insertBlock"],
  ["composer:insert-text", "insertText"],
  ["composer:replace-text", "replaceText"],
  ["composer:apply-surround", "_applySurround"],
  ["composer:indent-selected-text", "indentSelection"],
];
...
export default Component.extend(TextareaTextManipulation, {
  ...
  didInsertElement() {
    ...
    if (this.composerEvents) {
      for (const [event, handler] of COMPOSER_EVENTS) {
        this.appEvents.on(event, this, handler);
      }
    }
  },
  ...
  _shutDown() {
    if (this.composerEvents) {
      for (const [event, handler] of COMPOSER_EVENTS) {
        this.appEvents.off(event, this, handler);
      }
    }
  ...
  async _updatePreview() {
    ...
    if (this.siteSettings.enable_diffhtml_preview) {
      ...
      cookedElement.className = this._previewElement.className;
      ...
      const { morph } = await import("morphlex");
      morph(this._previewElement, cookedElement);
    }
